...
| USB retest | `MainWindow.xaml`, `MainWindow.xaml.cs` | The **Retest** button clears every USB port result, not only failures, preserves the detected port count, and restarts live monitoring. A technician must move the readable test drive through every port again after selecting it. |
...
| Diagnostics/report UI | `DiagnosticsResult.cs`, `QaSheetFiles.cs`, `QaSheetImageWindow.cs`, `macos/Services/DiagnosticsParser.cs` | Diagnostics results and QA sheet display/output. Auto-discovery prefers a fresh `DellPrebootDiagnosticsLog.txt`, then accepts a single retained `DellPrebootDiagnosticsLog*.txt` file or one whose filename matches the active service tag; it never silently chooses between multiple unmatched retained logs. Unanswered Dell interactive prompts are reported with their detected category (for example Video, Audio, Camera, Keyboard, or pointing device) rather than a generic warning. |
