...
4. For USB testing, follow the on-screen port indicators and move a readable test drive between the requested physical ports. Do not count a dock as a laptop port unless the workflow says to do so. The steps 1-7 reminder appears only after Laptop QA detects the expected USB-port count and every detected port has a result.
...
13. Open **Settings** to change any final-check destination under **Final Check Links**. The Update Stockrooms URL must retain `{SERIAL}` where the active laptop's service tag belongs. Windows also uses the **Upload Hash URL** after **Export Hash**. The macOS companion reads the shared links and supports the three cached final-check actions (Check Hash and Group Tag, Remove User, and Update Stockrooms); it does not collect or upload the Windows hash. Its buttons open the configured pages and copy the relevant information to the clipboard; they do not fill or submit forms.

### Settings and configuration

Use **Settings** for technician-level preferences and approved organization values. Technician name, language, and theme affect the header, session, and QA sheet. **Camera Roll folder** points the camera test and cleanup at the correct folder, and **Dell diagnostics log folder** is an optional location to search for preboot logs. The device-name format controls the name shown in the app, QA sheets, and saved files; it accepts `{serial}`, `{computer}`, and `{asset}`. The Autopilot group tag is the expected tag used when the exported hash is checked.

The Wi-Fi/Ethernet and Camera Roll timing fields control retry and rescan waits; leave them at the approved defaults unless your device procedure requires a change. ServiceNow settings identify the request page, request type, assignment group name and ID, and the browser-automation wait. If browser automation cannot start, Laptop QA still opens the request and copies the QA summary for manual completion. Final-check URLs are destinations, not credentials—never store passwords or access tokens in the configuration. **Reset Settings** restores defaults and removes the saved technician name without deleting QA sheets, logs, hardware files, or session history.
