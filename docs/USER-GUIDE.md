...
3. Run the displayed QA steps: battery, BIOS, diagnostics, camera/audio, display, keyboard, USB ports, storage, and other device-specific checks. When Windows steps 1-7 are complete, Laptop QA confirms that the session was saved and reminds you to move the QA USB drive to your own PC for the remaining final checks and QA logging.
...
8. Select **Export Hash** to collect and save the Windows Autopilot hardware hash. Select **Check Hash and Group Tag** to open Intune Windows Devices in a new Edge tab. The service tag is copied to the clipboard for a manual device search. Verify the device and group tag in Intune, then select the adjacent checkbox to mark the step complete.
