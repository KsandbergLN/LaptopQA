...
The **Final Check Links** in Config are shared: Check Hash and Group Tag, Remove User from Laptop in Intune, and Update Stockrooms open their configured Intune or ServiceNow page and copy the cached service tag. Keep `{SERIAL}` in the Update Stockrooms URL so the current tag can be inserted. The ServiceNow request button similarly copies the request description and opens the configured page. The macOS app does not fill or submit web forms; technicians paste the copied information where needed.
